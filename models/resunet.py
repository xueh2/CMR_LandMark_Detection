...
class GadgetronResUnet_UpSample(nn.Module):
    def __init__(self, block, layers, in_ch, out_ch, bilinear=True, stride=1, use_dropout=False, p=0.5, H=256, W=256, verbose=True):
        ...
        for i in range(self.layers):
            ...
            if i == 0:
                a_block = torch.jit.script(block(in_ch, out_ch, stride=stride, use_dropout=use_dropout, p=p, H=H_layer_in, W=W_layer_in, verbose=verbose))
                self.blocks.add_module(module_name, a_block)
            else:
                a_block = torch.jit.script(block(out_ch, out_ch, stride=stride, use_dropout=use_dropout, p=p, H=H_layer_in, W=W_layer_in, verbose=verbose))
                self.blocks.add_module(module_name, a_block)
    ...
    def forward(self, x1, x2):
        ...
        if self.bilinear:
            x1 = nn.functional.interpolate(x1, scale_factor=2.0, mode='bilinear', align_corners=True)
        else:
            x1 = self.up(x1)
...
class GadgetronResUnet(nn.Module):
    ...
    def __init__(self, block, 
                F0, inplanes, layers, layers_planes, 
                use_dropout=False, 
                p=0.5, 
                bilinear=True, 
                H=224, 
        ...
        if num_classes == 0 and num_outputs >0, the output layer is a FC layer to produce num_output values, for regression model 
        """
        ...
        # start up layer
        # --------------------------------------------------------------------
        self.input_layer = torch.jit.script(GadgetronResUnetInputBlock(F0, inplanes, stride=1, use_dropout=use_dropout, p=p, H=H, W=W, verbose=verbose))
        ...
        if(self.num_classes>0):
            ...
            if verbose:
                print("Output conv layer (%d, %d, %d) --> (%d, %d, %d)" % (input_planes, H_layer, W_layer, num_classes, H_layer, W_layer))
        
        if(self.num_outputs>0):
            self.output_FC = nn.Sequential()

            self.output_FC_conv_H = 8

            self.output_FC_conv = nn.Conv2d(input_planes, self.output_FC_conv_H, 1)
            self.output_FC_conv_BN = nn.BatchNorm2d(self.output_FC_conv_H)
            self.output_FC_conv_relu = nn.ReLU(inplace=True)

            self.output_FC1_H = int(20*self.num_outputs)

            self.output_FC.add_module("flatten", Flatten())

            self.output_FC.add_module("FC1", nn.Linear(self.output_FC_conv_H*H_layer*W_layer, self.output_FC1_H))
            self.output_FC[1].weight.data.normal_(0, math.sqrt(1. / (input_planes*H_layer*W_layer)))

            self.output_FC.add_module("FC1_RELU", nn.ReLU(inplace=True))

            self.output_FC.add_module("FC2", nn.Linear(self.output_FC1_H, self.num_outputs))
            self.output_FC[3].weight.data.normal_(0, math.sqrt(1. / self.output_FC1_H))

            if verbose:
                print("Output conv for FC layer (%d, %d, %d) --> (%d, %d, %d)" % (input_planes, H_layer, W_layer, self.output_FC_conv_H, H_layer, W_layer))
                print("Output FC1 layer (%d, %d, %d) --> (%d)" % (self.output_FC_conv_H, H_layer, W_layer, self.output_FC1_H))
                print("Output FC2 layer (%d) --> (%d)" % (self.output_FC1_H, self.num_outputs))
    ...
    def _make_down_layer(self, block, layers, inplanes, planes, stride=1, H_layer=64, W_layer=64, verbose=True):
        ...
        for i in range(layers):
            ...
            a_block = torch.jit.script(block(inplanes, planes, stride=stride, use_dropout=self.use_dropout, p=self.dropout_p, H=H_layer_in, W=W_layer_in, verbose=verbose))
            layer.add_module(block_name, a_block)
    ...
    def _make_up_layer(self, block, layers, inplanes, planes, stride=1, H_layer=64, W_layer=64, verbose=True):
        layer = torch.jit.script(GadgetronResUnet_UpSample(block, layers, inplanes, planes, self.bilinear, stride=stride, use_dropout=self.use_dropout, p=self.dropout_p, H=H_layer, W=W_layer, verbose=verbose))
    ...
    def forward(self, x):
        ...
        for l in range(num_layers+1):
            if l==num_layers:
                x = self.up_layers[l](x, x_input)
            else:
                x = self.up_layers[l](x, x_from_down_layers[num_layers-l-1])
       
        if(self.num_classes>0 and self.num_outputs==0):
            x = self.output_conv(x)
        
        if( (self.num_classes==0) and (self.num_outputs>0) ):
            x = self.output_FC_conv(x)
            x = self.output_FC_conv_BN(x)
            x = self.output_FC_conv_relu(x)
            x = self.output_FC(x)
        
        if(self.num_classes>0 and self.num_outputs>0):
            x1 = self.output_conv(x)
            
            x2 = self.output_FC_conv(x)
            x2 = self.output_FC_conv_BN(x2)
            x2 = self.output_FC_conv_relu(x2)
            x2 = self.output_FC(x2)
            return (x1, x2)

        return x

def GadgetronResUnet18(F0=3, inplanes=64, layers=[1,1,1], layers_planes=[128,256,512], use_dropout=False, p=0.5, H=224, W=224, C=2, num_outputs=0, verbose=True):
    """Constructs a GadgetronResUnet model.
    """
    model = GadgetronResUnet(GadgetronResUnetBasicBlock, F0, inplanes, layers, layers_planes, use_dropout=use_dropout, p=p, H=H, W=W, num_classes=C, num_outputs=num_outputs, verbose=verbose)
    return model
